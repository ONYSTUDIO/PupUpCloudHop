...
#### ⭐️ 로켓 모드 (별 아이템)
- [ ] 인게임 별 아이콘 스폰 시스템 구현 (위치, 등장 주기)
- [ ] 별 획득 충돌 판정 및 로켓 모드 진입 처리
- [ ] 로켓 모드 직선 고속 비행 이동 로직 (3초, 중력 무시)
- [ ] 로켓 모드 중 구름섬 통과 카운트 → 점수 부여
- [ ] 로켓 모드 중 디버프 무적 처리 (새 무리 / 번개 충돌 무시)
- [ ] 로켓 모드 진입 / 종료 비주얼 연출 (이펙트, 트레일)
- [ ] 로켓 모드 잔여 시간 HUD 표시 (3초 카운트다운)

#### 🫧 방어막 (비눗방울 보호막)
- [ ] 게임 시작 전 아이템 선택 UI 구현 (방어막 슬롯)
- [ ] 방어막 활성 상태 관리 (게임 시작 시 보호막 생성)
- [ ] 캐릭터 주변 원형 비눗방울 비주얼 렌더링
- [ ] 번개 충돌 시 방어막 1회 소멸 처리
- [ ] 새 무리 충돌 시 방어막 무시 (디버프 그대로 적용)
- [ ] 방어막 소멸 연출 (비눗방울 터지는 이펙트)
